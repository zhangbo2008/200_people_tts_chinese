...
class VarianceAdaptor(nn.Module):
    ...
    def forward(self,
        ...
        if duration_target is not None:
            duration_rounded = torch.clamp(torch.round((duration_target + self.duration_mean) * d_control), min=0) # 利用均值返回去算一个int
# duration_rounded 是真实的标签.
            x, mel_len = self.length_regulator(x, duration_rounded, max_len)  # 把结果拓展到每一帧.
        else:
